...
/*   Updated: 2024/11/28 15:22:37 by vagarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char **av, char *envp[])
{
    ...
    tools = NULL;
    tools->infile = open_file(av[1], O_RDONLY);
    tools->outfile = open_file(av[4], O_WRONLY | O_CREAT | O_TRUNC);
    tools = start_your_engines(av, tools->infile, tools->outfile, *envp);
    ...
    pid1 = fork();
    pid2 = fork();
    handle_pid1(tools, pid1, pid2, pipefd, *envp);
    // if ((pid1 = fork()) == -1)
    // {
    ...
    free_args(tools->cmd1_args);
    free_args(tools->cmd2_args);
    free(tools->cmd1_path);
    free(tools->cmd2_path);
    if (WIFEXITED(status)) {
